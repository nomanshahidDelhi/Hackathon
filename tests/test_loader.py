...
def test_full_plan_order():
    ...
    ops = [f"ops:{f.stem}" for f in sorted(lw.AGENT_OPS_SQL_DIR.glob("*.sql"))]
    assert keys[7:] == ops and ops[0] == "ops:01_agent_ops_schema"
...
def test_only_filter_excludes_agent_ops_unless_requested():
    ...
    assert len(lw.plan_steps(only={"ops"})) == len(list(lw.AGENT_OPS_SQL_DIR.glob("*.sql")))
